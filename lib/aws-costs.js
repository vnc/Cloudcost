...
var populateGrid = function(result, accountName) {
	...
	for (var i = 0; i < (result['reservationSet']['item']).length; i++) {
		...
		//specify last month's cost
		getEc2Cost(result['reservationSet']['item'][i]['instancesSet']['item']['instanceId'],
					{price: price,
					 startDate: new Date((new Date()).getFullYear(), (new Date()).getMonth()-1, 1),
					 //startDate: new Date(new Date((new Date()).setDate(1)).setMonth((new Date()).getMonth - 1)),
					...
					function(instanceId, calculatedCost) {
						...
						for (var x = 0; x < dataGrid.rows.length; x++) {
							if (dataGrid.rows[x].cell[ID] == id) {
								dataGrid.rows[x].cell[LAST_MONTH_COST] = calculatedCost;
								console.log("instanceId " + id + " cost:   " + calculatedCost)
							}
						}
					});
		...
		getEc2Cost(result['reservationSet']['item'][i]['instancesSet']['item']['instanceId'],
					{price: price,
					 startDate: new Date(new Date((new Date()).setDate(1))).setMonth(0)
					},
					function(instanceId, calculatedCost) {
						var id = instanceId;
						for (var x = 0; x < dataGrid.rows.length; x++) {
...
var getEc2Cost = function(instanceId, props, callback) {
	...
	var endDate = props.endDate || (new Date()).getTime();
		...
		for (var z = 1; z < result.length; z++) {
			...
			if (currentItem.action == 'stopped' || currentItem.action == 'terminated') {
				if (priorItem.action == 'started' || priorItem.action == 'created') {
					if (currentItem.date >= startDate && priorItem.date < endDate){
						var theDate = (priorItem.date <= startDate) ? startDate : priorItem.date;
						var theEndDate = (currentItem.date > endDate) ? endDate : currentItem.date;
						milliseconds = milliseconds + (theEndDate - theDate);
					}
				}
			}
		}
...
exports.getInstances = function(queryObj, callback) {
	...
	if (query && query != "" && query != "undefined" && query != undefined) {
		...
		for (var p = 0; p < (dataGridCopy.rows).length; p++) {
			...
			if (cellContents && cellContents != undefined && cellContents != "undefined" && typeof(cellContents) != 'object') {
	...
	dataGridCopy.rows.sort(function(a,b) {
		...
		switch(sortname) {
			...
			case 'LAST_MONTH_COST':
				diff = parseFloat(a.cell[LAST_MONTH_COST]) - parseFloat(b.cell[LAST_MONTH_COST]);
				if (isNaN(b.cell[LAST_MONTH_COST])) diff = 1;
				break;
			case 'YEAR_COST':
	...
	for (var k = 0; k < (dataGridCopy.rows).length; k++){
		...
		lastMonthCost = lastMonthCost + parseFloat(dataGridCopy.rows[k].cell[LAST_MONTH_COST]);
	}
	...
	dataGridCopy.lastMonthCost = "$" + addCommas(lastMonthCost.toFixed(0));
